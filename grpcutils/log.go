...
func unaryClientLoggerInterceptor() func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
	return func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		if err != nil {
			logging.Debugc(ctx, "Failed to invoke method %s invoke_time=%s invoke_err=%s", method, td(), err)
		} else {
			logging.Debugc(ctx, "Succeed to invoke method %s invoke_time=%s", method, td())
		}
		return err
	}
}
