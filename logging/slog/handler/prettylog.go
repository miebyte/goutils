...
func (h *PrettyHandler) formatTextAttrs(attrs []OrderedAttr) ([]byte, error) {
...
func (h *PrettyHandler) formatTermAttrs(attrs []OrderedAttr) ([]byte, error) {
	attrMap := make(map[string]any, len(attrs))
	for _, attr := range attrs {
		attrMap[attr.Key] = attr.Value
	}

	attrsBytes, err := json.MarshalIndent(attrMap, "", "  ")
	if err != nil {
...
func (h *PrettyHandler) formatAttrs(r slog.Record) ([]OrderedAttr, error) {
...
func (h *PrettyHandler) addPredefAttrs(attrs *[]OrderedAttr) {
...
func (h *PrettyHandler) addRecordAttrs(attrs *[]OrderedAttr, r slog.Record) {
...
func (h *PrettyHandler) addAttrToGroup(current map[string]any, attr slog.Attr) {
	temp := current

	for i, g := range h.groups {
		if i == len(h.groups)-1 {
			h.addAttrToFinalGroup(temp, g, attr)
		} else {
			temp = h.ensureNestedGroup(temp, g)
		}
	}
}
