// File:		slog.go
// Created by:	Hoven
// Created on:	2025-04-03
//
// This file is part of the Example Project.
//
// (c) 2024 Example Corp. All rights reserved.

package slog

import (
	"context"
	"fmt"
	"io"
	"log/slog"
	"os"
	"runtime"
	"strings"

	"github.com/miebyte/goutils/internal/buildinfo"
	"github.com/miebyte/goutils/logging/level"
	"github.com/miebyte/goutils/logging/logctx"
	"github.com/miebyte/goutils/logging/parser"
	"github.com/miebyte/goutils/logging/slog/handler"
	"github.com/miebyte/goutils/logging/writer"
	"github.com/segmentio/kafka-go"
)

type logable func(ctx context.Context, msg string, args ...any)

type Logger struct {
	logger          *slog.Logger
	logLevel        level.Level
	slogLoggerLevel *slog.LevelVar
	slogOpt         *slog.HandlerOptions
	callDepth       int
	withHost        bool
	withSource      bool
}

type Option func(*Logger)

func WithCalldepth(d int) Option {
	return func(l *Logger) {
		l.callDepth = d
	}
}

func WithHost() Option {
	return func(l *Logger) {
		l.withHost = true
	}
}

func WithSource() Option {
	return func(l *Logger) {
		l.withSource = true
	}
}

func New(opts ...Option) *Logger {
	return NewSlogPrettyLogger(os.Stdout, opts...)
}

func newWithHandler(handler slog.Handler, lev *slog.LevelVar, opt *slog.HandlerOptions, opts ...Option) *Logger {
	if opt == nil {
		opt = &slog.HandlerOptions{}
	}

	if opt.AddSource {
		opts = append(opts, WithSource())
		opt.AddSource = false
	}
	opt.Level = lev
	logger := slog.New(handler)
	l := &Logger{
		logger:          logger,
		logLevel:        level.LevelInfo,
		slogLoggerLevel: lev,
		slogOpt:         opt,
		callDepth:       5,
	}

	for _, opt := range opts {
		opt(l)
	}

	return l
}

func NewSlogPrettyLogger(w io.Writer, opts ...Option) *Logger {
	lev := &slog.LevelVar{}
	lev.Set(slog.LevelInfo)
	slogOpt := &slog.HandlerOptions{
		Level: lev,
	}

	handler := handler.NewPrettyHandler(w, slogOpt)
	return newWithHandler(handler, lev, slogOpt, opts...)
}

func NewSlogTextLogger(w io.Writer, opts ...Option) *Logger {
	lev := &slog.LevelVar{}
	lev.Set(slog.LevelInfo)
	slogOpt := &slog.HandlerOptions{
		Level: lev,
	}
	handler := slog.NewTextHandler(w, slogOpt)
	return newWithHandler(handler, lev, slogOpt, opts...)
}

func NewSlogJsonLogger(w io.Writer, opts ...Option) *Logger {
	lev := &slog.LevelVar{}
	lev.Set(slog.LevelInfo)
	slogOpt := &slog.HandlerOptions{
		Level: lev,
	}
	handler := slog.NewJSONHandler(w, slogOpt)
	return newWithHandler(handler, lev, slogOpt, opts...)
}

func NewSlogKafkaLogger(kafkaWriter *kafka.Writer, opts ...Option) *Logger {
	w := writer.NewKafkaLogWriter(kafkaWriter)
	logger := New(opts...)
	logger.SetOutput(w)

	return logger
}

func (sl *Logger) getSrouce() (string, int) {
	_, file, line, _ := runtime.Caller(sl.callDepth)
	modulePath := buildinfo.GetModulePath()
	if modulePath == "" {
		return file, line
	}

	_, after, ok := strings.Cut(file, modulePath)
	if !ok {
		return file, line
	}

	file = modulePath + after

	return file, line
}

func (l *Logger) Enable(lev level.Level) {
	l.logLevel = lev
	l.slogLoggerLevel.Set(slog.Level(lev))
}

func (l *Logger) IsDebug() bool {
	return l.logLevel == level.LevelDebug
}

func (l *Logger) SetOutput(w io.Writer) {
	handler := slog.NewJSONHandler(w, l.slogOpt)
	l.logger = slog.New(handler)
}

func (l *Logger) logc(c context.Context, lev level.Level, lb logable, msg string, v ...any) {
	lc := logctx.GetLogContext(c)
	if lc == nil {
		lc = &logctx.LogContext{}
	}

	msg, args := l.logFmt(lc, msg, v...)

	lb(c, msg, args...)
}

func (l *Logger) logFmt(lc *logctx.LogContext, msg string, v ...any) (string, []any) {
	s, keys, values, err := parser.ParseFmtKeyValue(msg, v...)
	if err != nil {
		return msg + " " + err.Error(), nil
	}

	msg = s
	keys = append(keys, lc.Keys...)
	values = append(values, lc.Values...)

	var builder strings.Builder
	for _, g := range lc.Group {
		builder.WriteString("[")
		builder.WriteString(g)
		builder.WriteString("] ")
	}

	builder.WriteString(msg)

	if !strings.HasSuffix(msg, ".") && !strings.HasSuffix(msg, "。") {
		builder.WriteString(".")
	}
	builder.WriteString(" ")

	args := l.parseArgs(keys, values)
	return builder.String(), args
}

func (l *Logger) parseArgs(keys, values []string) []any {
	var args []any

	if l.withSource {
		file, line := l.getSrouce()
		args = append(args, "source", fmt.Sprintf("%s:%d", file, line))
	}

	if l.withHost {
		args = append(args, "host", buildinfo.GetHost())
	}

	for idx, key := range keys {
		args = append(args, key, values[idx])
	}

	logctx.ReverseGroupWithPairs(args)
	return args
}

func (l *Logger) Infoc(ctx context.Context, msg string, v ...any) {
	l.logc(ctx, level.LevelInfo, l.logger.InfoContext, msg, v...)
}

func (l *Logger) Debugc(ctx context.Context, msg string, v ...any) {
	l.logc(ctx, level.LevelDebug, l.logger.DebugContext, msg, v...)
}

func (l *Logger) Warnc(ctx context.Context, msg string, v ...any) {
	l.logc(ctx, level.LevelWarn, l.logger.WarnContext, msg, v...)
}

func (l *Logger) Errorc(ctx context.Context, msg string, v ...any) {
	l.logc(ctx, level.LevelError, l.logger.ErrorContext, msg, v...)
}

func (l *Logger) Fatalc(ctx context.Context, msg string, v ...any) {
	l.logc(ctx, level.LevelError, l.logger.ErrorContext, msg, v...)
	os.Exit(1)
}

func (l *Logger) Infof(msg string, v ...any) {
	l.logc(context.TODO(), level.LevelInfo, l.logger.InfoContext, msg, v...)
}

func (l *Logger) Debugf(msg string, v ...any) {
	l.logc(context.TODO(), level.LevelDebug, l.logger.DebugContext, msg, v...)
}

func (l *Logger) Warnf(msg string, v ...any) {
	l.logc(context.TODO(), level.LevelWarn, l.logger.WarnContext, msg, v...)
}

func (l *Logger) Errorf(msg string, v ...any) {
	l.logc(context.TODO(), level.LevelError, l.logger.ErrorContext, msg, v...)
}

func (l *Logger) Fatalf(msg string, v ...any) {
	l.logc(context.TODO(), level.LevelError, l.logger.ErrorContext, msg, v...)
	os.Exit(1)

}

func (l *Logger) PanicError(err error, v ...any) {
	if err == nil {
		return
	}

	var s string
	if len(v) > 0 {
		s = err.Error() + ":" + fmt.Sprint(v...)
	} else {
		s = err.Error()
	}
	l.logc(context.TODO(), level.LevelError, l.logger.ErrorContext, s)
	panic(err)
}
