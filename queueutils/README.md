...
queueutils 是一个轻量级、灵活的 Go 队列库，提供多种队列实现方式，包括内存队列、优先级队列和 Redis 队列。
...
```bash
go get github.com/miebyte/goutils/queueutils
```
...
import (
    "fmt"
    "github.com/miebyte/goutils/queueutils"
)

func main() {
    q := queueutils.NewMemoryQueue[string]()
...
func main() {
    // 创建高优先级优先的队列
    pq := queueutils.NewPriorityQueue[*Task](
        queueutils.WithPriorityMode(queueutils.HighPriorityFirst),
    )
...
import (
    "fmt"
    "github.com/go-puzzles/puzzles/queueutils"
)
...
- `NewRedisQueueWithClient[T Item](client *redisutils.RedisClient, queue string) *RedisQueue[T]`: 使用现有客户端创建队列
