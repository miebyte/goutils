...
func NewRedisQueue[T Item](addr string, db int, queue string) (*RedisQueue[T], error) {
	...
	redisclient, err := redisutils.NewRedisClient(conf)
	if err != nil {
		return nil, err
	}
...
func NewRedisQueueWithClient[T Item](client *redisutils.RedisClient, queue string) *RedisQueue[T] {
	return &RedisQueue[T]{
		client: client,
		queue:  queue,
		ctx:    context.Background(),
